...
This is a famous data set from 1936, courtesy of Ronald Fisher, that comes with R and is excellent for demonstrating ggplot2.
...
A common example is adding summary stats to plots.

## ggplot2 example - adding means to boxplots

First we calculate mean `Sepal.Width` by `Species`.    
The `aggregate` function returns a data frame.
...
```{r, message=FALSE, fig.height=3, fig.width=6}
ggplot(iris, aes(x=Species, y=Sepal.Width)) + 
  geom_boxplot() + geom_point(data=irisMeans, color="red")
```
